...
const Workinghoursl = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://apidev.myinteriormart.com/api/WorkingHours/WorkingHours', {
                method: 'POST',
                headers: {
    ...
    <div className='row'>
        {Object.keys(workingHours).map((key) => (
            <div key={key} className='col-md-6'>
                {key === 'SaturdayHoliday' || key === 'SundayHoliday' ? null : (
                    <div className='form-group'>
                        <label>
                            {key}
                            <input
                                className="form-control form-control-sm"
                                type={key.includes('Holiday') ? 'checkbox' : 'time'}
                                name={key}
                                checked={key.includes('Holiday') ? workingHours[key] : undefined}
                                value={!key.includes('Holiday') ? workingHours[key] : undefined}
